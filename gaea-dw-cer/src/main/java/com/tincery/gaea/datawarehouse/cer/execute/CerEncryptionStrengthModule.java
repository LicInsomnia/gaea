...
//            System.load("C:\\Users\\Insomnia\\Documents\\Visual Studio 2010\\Projects\\helloTest\\x64\\Debug\\helloTest.dll");
            while (true) {
                ...
                if (cer != null) {
                    if(cer.getSubjectPublicKeyInfoAlgorithmOid().equals("1.2.840.113549.1.1.1")) {
                    ...
                    } else if(cer.getSubjectPublicKeyInfoAlgorithmOid().equals("1.2.840.10045.2.1")) {
                        String xStr = cer.getEccX();
                        String yStr = cer.getEccY();
                        String datPath = NodeInfo.getConfig() + "/pubkeyCheck/";
                        cer.setKeyExchangeLeakDetectResult(keyExchangeLeakDetect(xStr, yStr, datPath) ? 1 : 0);
                    }
                    queueOutput.put(cer);
                }
                if (queueInput.isEnd()) {
